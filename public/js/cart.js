    ...
    function getCart() {
      ...
      if (cart.length) {
        cartIndicator.style.cssText = `
          ...
          background-color: #ff575c; border-radius: 100%; position: absolute; top:-12px; font-size: 12px;
  font-weight: bold; right:-15px;
        `;
    ...
    // Add item
    function addToCart(id, name, img_url, price, qty = 1, callCheckout = false) {
      const cart = getCart();
      ...
      Toast.success('Added to Cart!',`${!name ? existing.name : name}`);
